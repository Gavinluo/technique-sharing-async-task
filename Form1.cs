...
namespace TechniqueSharingAsyncTask
{
	public partial class Form1 : Form
	{
		...
		private async void btnTaskWork_Click(object sender, EventArgs e)
		{
			...
			// Add progress handler that also occasionally logs so user sees activity in lstLog.
			var progress = new Progress<int>(step =>
			{
				// Update status bar every step.
				UpdateStatus(runId, step, 300);
				Log($"[Run#{runId}] Step {step}/300 (progress)");
			});
